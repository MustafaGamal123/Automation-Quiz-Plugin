...
foreach ($quizsummary as $quiz) {
    if ($course->id == $quiz->courseid) {
        ...
        // Add Warning action to the action menu.
        $warningurl = new moodle_url('/mod/quiz/accessrule/proctoring/warnings.php', [
            'cmid' => $cmid,
            'quizid' => $quiz->quizid,
            'sesskey' => sesskey(),
        ]);
        ...
        $warningaction = new action_menu_link_secondary(
            $warningurl,
            new pix_icon('i/warning', get_string('warnings', 'quizaccess_proctoring')),
            get_string('warnings', 'quizaccess_proctoring')
        );
